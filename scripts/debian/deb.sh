...
if [ true ]; then
	cd builder
	wget https://github.com/luigifab/qt-style-globalqss/archive/v$version/qt-style-globalqss-$version.tar.gz
	tar xzf qt-style-globalqss-$version.tar.gz
	cd ..
else
	temp=qt-style-globalqss-$version
	mkdir /tmp/$temp
	cp -r ../../* /tmp/$temp/
...
for serie in experimental resolute questing noble jammy mx25 mx23 mx21; do

	printf "\n\n#################################################################### $serie ##\n\n"
	if [ $serie = "experimental" ]; then
		# copy for Ubuntu
		cp -a builder/qt-style-globalqss-$version/ builder/qt-style-globalqss-$version+src/
		cd builder/qt-style-globalqss-$version/
	elif [ $serie = "unstable" ]; then
		rm -rf builder/qt-style-globalqss-$version/
		cp -a builder/qt-style-globalqss-$version+src/ builder/qt-style-globalqss-$version/
		cd builder/qt-style-globalqss-$version/
	else
		cp -a builder/qt-style-globalqss-$version+src/ builder/qt-style-globalqss-$serie-$version/
		cd builder/qt-style-globalqss-$serie-$version/
	fi

	dh_make -s -y -f ../qt-style-globalqss-$version.tar.gz -p qt-style-globalqss
